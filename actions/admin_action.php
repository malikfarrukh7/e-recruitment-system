...
function updateJob($db) {
    ...
    if ($job->updateJob()) {
        header("Location: ../dashboard/ADboard//manage-jobs.php?success=job_updated");
    } else {
        header("Location: ../pages/admin_dashboard.php?error=job_update_failed");
    }
}
...
function deleteJob($db) {
    ...
    if ($job->deleteJob()) {
        header("Location: ../dashboard/ADboard//manage-jobs.php?success=job_deleted");
    } else {
        header("Location: ../pages/admin_dashboard.php?error=job_delete_failed");
    }
}
...
function changeJobStatus($db, $new_status) {
    ...
    if ($job->updateStatus()) {
        header("Location: ../dashboard/ADboard/manage-jobs.php?success=job_status_changed");
    } else {
        header("Location: ../pages/admin_dashboard.php?error=job_status_change_failed");
    }
}

function fetchCalendarData($admin) {
    $calendarData = $admin->getCalendarData();
    if (empty($calendarData)) {
        echo json_encode(["error" => "No data found in calendar"]);
        return;
    }
    ...
    foreach ($calendarData as $entry) {
        $events[] = [
            'title' => "{$entry['job_name']} ({$entry['interview_count']} interviews)",
            'start' => "{$entry['interview_date']}T{$entry['interview_time']}",
            'extendedProps' => [
